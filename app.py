...
if portfolio:
    ...
    m1.metric("PORTFOLIO VALUE", f"${total_val:,.2f}", delta=f"{drawdown:.2f}% vs Peak", delta_color="normal")
    m2.metric("HIGH WATER MARK", f"${hwm:,.2f}")
    m3.metric("95% VaR (Daily)", f"${total_val * 0.027:,.2f}")
    m4.metric("MAX DRAWDOWN", f"{max_dd:.2f}%", delta="Historical Risk", delta_color="off")
    ...
    # --- ROW 4: REBALANCING & DRIFT LEDGER ---
    st.divider()
    ...
    for i, asset in enumerate(TARGETS.keys()):
        ...
        status = current_pos[asset] 
        with d_cols[i]:
            if st.session_state.trades_done[asset]:
                st.success(f"**{asset} SESSION COMPLETE**")
            elif var_need > 50:
                if status == "🔴 RESISTANCE":
                    st.error(f"**{asset} HOLD ENTRY**\nPrice is at Resistance. Market buy blocked.")
                elif status == "🟡 HOLD":
                    st.warning(f"**{asset} CAUTION**\nWait for {asset} pullback to ${LEVELS[asset]['support']*1.05:,.2f}.")
                else: # 🟢 ACCUMULATION
    ...
    st.error("🛑 API Connection Failed. Check .env credentials and internet.")